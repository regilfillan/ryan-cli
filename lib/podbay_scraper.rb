...
class PodbayScraper
  ...
  def self.scrape_details(podcast)
    ...
    podcast.open_website= details[7].attribute('href').value
    podcast.read_reviews= details[4].attribute('href').value
    podcast.episode_list= details[2].attribute('href').value
  end
